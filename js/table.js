class Table {
    ...
    getAllTableDataCells() {
        return this.table.querySelectorAll("td");
    }
    getAllTableHeaderCells() {
        return this.table.querySelectorAll("th");
    }
    fillEmptyCells() {
        const NBSP = "\xa0";
        this.getAllTableDataCells().forEach(cell => { if (cell.innerText === "")
            cell.innerText = NBSP; });
    }
    loadTableStyles() {
        const cells = [...this.getAllTableDataCells(), ...this.getAllTableHeaderCells()];
    ...
    getStyleAttributesAndRewrite(cell) {
